...
if selected_mode == "chat":
    ...
    def render_canvas():
        with st.container(border=True):
            ...
            # add it to the history as well
            add_artifacts(st.session_state.canvas_title, st.session_state.canvas_kind, st.session_state.canvas_lang, st.session_state.canvas_content)
    ...
    with st.expander('Artifacts History', expanded=False):
        ...
        if not artifacts:
            ...
            for a in artifacts:
                ...
                with container:
                    ...
                    for title, kind, lang, content, created_at in artifacts:
                        st.subheader(f"{title} ({kind}, {lang})")
                        st.caption(f"Created at: {created_at}")
                        if lang == "html":
                            components.html(content, height=300, scrolling=True)
                        elif lang in ["python", "javascript", "java", "c++", "c#", "ruby", "go"]:
                            st.code(content, language=lang)
